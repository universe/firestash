...
interface RocksDbConstructor {
  (location: string): LevelDown; // RocksDB types are borked. Use this instead.
}


let levelup: LevelUp.LevelUpConstructor | undefined;
try { levelup = (await import('levelup')).default as unknown as LevelUp.LevelUpConstructor; }
catch { 1; }

let leveldown: LevelDownConstructor | undefined;
try { leveldown = (await import('leveldown')).default as unknown as LevelDownConstructor; }
catch { 1; }

let memdown: typeof MemDown | undefined;
try { memdown = (await import('memdown')).default as unknown as typeof MemDown; }
catch { 1; }

let rocksdb: RocksDbConstructor | undefined;
try { rocksdb = (await import('rocksdb')).default as unknown as RocksDbConstructor; }
catch { 1; }
