...
function reify<T = object>(data: Buffer | null) { return data ? JSON.parse(data.slice(1).toString()) as T : null; }
function isDirty(data: Buffer): boolean { return data[0] === 48; } // Character code for "1";
...
export default class FireStash extends AbstractFireStash {
        ...
        for (const [ key, objects ] of keys.entries()) {
          ...
          for (const obj of objects.length ? objects : [null]) {
            if (obj === DELETE_RECORD) {
              ...
              if (!this.options.lowMem) {
                const localObj = await this.safeGet(collection, key) || Buffer.from('1{}');
  ...
  private async drainEvents() {
    console.log('DRAIN', this.localEvents.size);
